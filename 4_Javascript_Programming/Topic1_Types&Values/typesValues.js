/*
	...
	JavaScript is a loosely typed or a dynamic language. Variables in JavaScript are 
...
/*
    ...
    2) Objects are aggregations of properties. A property can reference an object or a primitive. A primitive 
       (primitive value, primitive data type) is data that is not an object and has no properties.
...
/*
    The global NaN property is a value representing Not-A-Number. window['NaN']
    Infinity is a property of the global object, i.e. it is a variable in global scope.
    The IEEE 754 spec for floating-point numbers (which is used by all languages for 
      floating-point) says that NaNs are never equal. JavaScript spec follows this standard.
*/
...
/*
    ...
    than the global scope, as shown below...
*/
...
/*
    ...
    Recommendation is to never use null instead use undefined.
*/

var foo = null; // foo's value is of 'Null' primitive type, where the variable's value is Javascript's primitive value 'null'.

/*
    why is typeof null “object”?

    A) In the first implementation of JavaScript, JavaScript values were represented 
    as a type tag and a value, with the type tag for objects being 0, and null 
    was represented as the NULL pointer (0x00 on most platforms). As a result, 
    null had 0 as a type tag, hence the bogus typeof return value (reference).
...
/*
    ...
    you’ll see that two escapes are needed for each astral symbol. This is confusing, because humans generally think in 
    terms of Unicode symbols
*/
var str7 = '\uD83D\uDCA9';
...
var float_var = 3.141592653589793;
...
a limited set of properties are initialized; then properties can be added and removed. Property 
values can be values of any type, including other objects, which enables building complex data 
structures. Properties are identified using key values. A key value is either a String or a Symbol value.
...
[[Enumerable]] 		Boolean 		If true, the property will be enumerated in for...in loops. 		true
[[Configurable]] 	Boolean 		If false, the property can't be deleted and attributes other 		true
